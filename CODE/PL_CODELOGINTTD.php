	...
	//include"../../CONFIG/MYSQL/MY_CONNECT_PRD.php";
	  if(isset($_POST['hr_acs_login'])){ 
			...
			$mdpass =EN_HS_MD5($passuser);
				$dt=$CL_Q($CONN01,"$CL_SL pl_srv_01.srv_user_01 WHERE user_nama_01='$username' AND user_pass_01='$mdpass'");
					$bc=$CL_FR($dt);
			if($bc > 0){
				  // session_start(); // di gunakan untuk mengawali perintah session
				   ...
				   $_SESSION['passuser']=$bc[3];
					...
					?>
					<META HTTP-EQUIV="Refresh" Content="0; URL=<?PHP echo"PL_TTD_LIST_01.php"; ?>">
